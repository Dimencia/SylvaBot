...
public class MessageReplaceParser {
	...
	public static String parseMessage(String channel, String sender,
		...
		if (message.contains("(_RANDUSER_)")) {
			User[] users = ReceiverBot.getInstance().getUsers(ReceiverBot.getInstance().targetedChannel);
			message = message.replace("(_RANDUSER_)", users[(int)Math.floor(Math.random()*users.length)].getNick());
		}
