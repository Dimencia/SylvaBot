...
public class ReceiverBot extends PircBot {
	public static ReceiverBot instance;
	Timer joinCheck;
	...
	private ChatterBotSession bot1session = null;
	
	public final String targetedChannel = "#sylvacoin";
	private int currencyPerMinute = 1;
	private int currencyLiveMultiplier = 1; // Set to 1 since we want .5 coins per minute while live, .25 while not, and it loops each minute
	private HashMap<String,Integer> currencyMap = new HashMap<String,Integer>();
	...
	private String raffleSponsor = "SylvaCoin";
	private int raffleCost = 0;
	private boolean raffleRunning = false;
	private ArrayList<String> interestingQuotes = new ArrayList<String>();
	private ArrayList<String> interestingLeadIns = new ArrayList<String>();
	private int interestingQuoteDelay = 60000*30; // In miliseconds - this is 30 minutes
	private boolean wasLive = false;
	private int raidReward = 300;
	private ArrayList<String> currentHosts = new ArrayList<String>();
	private ArrayList<String> oldHosts = currentHosts;
	private ArrayList<String> hostsThisSession = oldHosts;
	private int hostReward = 0;
	private int hostRewardPerViewer = 10;
	private GreenScreen gs;
	private ArrayList<String> recentFollows = new ArrayList<String>();
	private ArrayList<String> oldRecentFollows = recentFollows;
	private int coinSkipCounter = 0;
	private int followCoins = 142;
	private String donationFilePath = "C:\\Users\\sterling\\Desktop\\Stream\\twitchalerts\\session_donators.txt";
	private ArrayList<String> oldDonators = new ArrayList<String>();
	private boolean isFinishedReadingCurrency = false;
	...
	public ReceiverBot(String server, int port) {
        ...
        final TrayIcon trayIcon =
                new TrayIcon(createImage("botIcon.jpg", "tray icon"));
        final SystemTray tray = SystemTray.getSystemTray();
       
        // Create a pop-up menu components
        MenuItem screenItem = new MenuItem("Green Screen");
        ...
        exitItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				writeCurrencyToFile("currencyInfo.txt");
		...
		readRanksFromFile();
		readInterestingQuotesFromFile();
		
		gs = new GreenScreen();
		
		recentFollows = JSONUtil.getRecentFollowers(targetedChannel.substring(1).toLowerCase());
		oldRecentFollows = recentFollows;
		
		Runnable donationChecker = new Runnable() { 
			public void run() {
				FileReader fr = null;
				BufferedReader tr = null;
				try {
					boolean isLive = JSONUtil.krakenIsLive(targetedChannel.substring(1));
					ArrayList<String> newDonators = new ArrayList<String>();
					fr = new FileReader(donationFilePath);
					...
					tr.close();
					System.out.println("Detected " + newDonators.size() + " total donators; compared to " + oldDonators.size() + " old donators");
					if(newDonators.size() != oldDonators.size()) {
						System.out.println("Checking which of these donators is new since those two numbers aren't the same...");
						if(isLive)
							for(int i = oldDonators.size(); i < newDonators.size(); i++) {
								String[] results = newDonators.get(i).split(": ");
								String name = results[0];
								System.out.println("Donator " + name + " is new, doing stuff with that.");
								double value = Double.parseDouble(results[1].replace("$", ""));
								send(targetedChannel, name + " just traded " + results[1] + " for " + value*100 + " coins!  Thank you so much :D");
								int oldCoins = 0;
								try{
									oldCoins = currencyMap.get(name.toLowerCase());
								} catch (Exception e) { e.printStackTrace(); }
								currencyMap.put(name.toLowerCase(), oldCoins + (int)(value*100));
							}
						else
							System.out.println("Want to announce a new donator, but channel isn't live - this is probably during startup");
					}
					oldDonators = newDonators;
					
					Thread.sleep(30000);  // 30 seconds should be fine.
				}
				catch (Exception e){
					e.printStackTrace();
					return;
		...
		Thread donationThread = new Thread(donationChecker);
		donationThread.start();
		
		
		Runnable currencyAdder = new Runnable() {
			@Override
			public void run() {
				boolean skipCoin = true;
				boolean isLive = JSONUtil.krakenIsLive(targetedChannel.substring(1));
				if((coinSkipCounter >= 1 && isLive) || (coinSkipCounter >= 3 && !isLive))
					skipCoin = false;
				else 
					skipCoin = true;
				for(User u : instance.getUsers(targetedChannel)) {
					String curNick = u.getNick().toLowerCase();
				
					if(!currencyMap.keySet().contains(curNick))
						currencyMap.put(curNick,0);
					
					// Check if Live... Note, maybe also check if following if sylva wants it
					if(isLive)
					{
						if(!skipCoin)
						{
							currencyMap.put(curNick, currencyMap.get(curNick) + (currencyPerMinute*currencyLiveMultiplier));
							skipCoin = false;
							System.out.println("adding coin");
						}
						else
						{
							System.out.println("Skipping adding coin");
							skipCoin = true;
						}
						wasLive = true;
					}
					else
					{
						if(wasLive) {
							wasLive = false;
							send(targetedChannel,"BibleThump Bye Sylva!");
							currentHosts = new ArrayList<String>(); // Resetting these may not be necessary but session hosts is
							oldHosts = currentHosts;
							hostsThisSession = oldHosts;
						} // Reset all command counts if he really wants that.  Check with him.
						// But this is where we can do anything we want the moment we detect he's stopped strim
						if(!skipCoin) {
							currencyMap.put(curNick, currencyMap.get(curNick) + (currencyPerMinute));
							skipCoin = false;
							//System.out.println("adding coin");
						}
						else {
					...
					// And then add time spent in the channel.  Check with sylva if he wants to count time spent while offline.
					if(!timeMap.keySet().contains(curNick))
						timeMap.put(curNick, 0);
					String rank = getUserRank(curNick);
					timeMap.put(curNick, timeMap.get(curNick)+1); // TimeMap counts minutes.
					String newRank = getUserRank(curNick);
					if(newRank != rank && !newRank.equalsIgnoreCase("coinless")) {
						send(targetedChannel, u + " just ranked up!  You are now a " + newRank + "!");
					}
				}
				if(skipCoin)
					coinSkipCounter++;
				...
				Thread currencyThread = new Thread(this);
				currencyThread.start();
			}
		};
		
		Thread currencyThread = new Thread(currencyAdder);
		currencyThread.start();
		
		Runnable currencySaver = new Runnable() {

			@Override
			public void run() {
				try {
					Thread.sleep(60000 * 10);// save every 10 mins
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				writeCurrencyToFile("currencyInfo.txt");
				System.out.println("Writing currency info");
				Thread currencySaverThread = new Thread(this);
				currencySaverThread.start();
			}
			
		};
		
		Thread currencySaverThread = new Thread(currencySaver);
		currencySaverThread.start();
		
		Runnable interestingQuoter = new Runnable() {

			@Override
			public void run() {
				try {
					Thread.sleep(interestingQuoteDelay);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				boolean isLive = JSONUtil.krakenIsLive(targetedChannel.substring(1));
				if(isLive)
					send(targetedChannel,interestingLeadIns.get((int)Math.floor(Math.random()*(interestingLeadIns.size()+1))) + interestingQuotes.get((int)Math.floor(Math.random()*(interestingQuotes.size()+1))));
		...
		Thread interestingQuoterThread = new Thread(interestingQuoter);
		interestingQuoterThread.start();
		
		Runnable hostUpdater = new Runnable() {

			@Override
			public void run() {
				boolean isLive = JSONUtil.krakenIsLive(targetedChannel.substring(1));
				currentHosts = JSONUtil.getHosts(targetedChannel.substring(1).toLowerCase());
				if(!currentHosts.equals(oldHosts) && isLive)
					updateHosts();
				recentFollows = JSONUtil.getRecentFollowers(targetedChannel.substring(1).toLowerCase());
				
				System.out.print("Recent follows: ");
				for(String s : recentFollows) {
					System.out.print(s + ", ");
				}
				System.out.println();
				
				if(!recentFollows.equals(oldRecentFollows))
					updateFollows();
				
				try {
					Thread.sleep(61000); // Twitch says you should poll once per minute at the most, we'll do 61 seconds to be safe
				} catch (Exception e) {
					e.printStackTrace();
				}
				Thread t = new Thread(this);
		...
		Thread hostUpdateThread = new Thread(hostUpdater);
		 ...
		 */ // Old schedule code, just in case
		...
		symbolsPatterns[0] = Pattern
				.compile("(\\p{InPhonetic_Extensions}|\\p{InLetterlikeSymbols}|\\p{InDingbats}|\\p{InBoxDrawing}|\\p{InBlockElements}|\\p{InGeometricShapes}|\\p{InHalfwidth_and_Fullwidth_Forms}|ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤|Ãƒâ€šÃ‚Â°|á»ŽÌ·Í–ÍˆÌžÌ©ÍŽÌ»Ì«Ì«ÌœÍ‰Ì Ì«Í•Ì­Ì­Ì«Ì«Ì¹Ì—Ì¹ÍˆÌ¼Ì Ì–Í�ÍšÌ¥ÍˆÌ®Ì¼Í•Ì Ì¤Ì¯Ì»Ì¥Ì¬Ì—Ì¼Ì³Ì¤Ì³Ì¬ÌªÌ¹ÍšÌžÌ¼Ì Í•Ì¼Ì Ì¦ÍšÌ«Í”Ì¯Ì¹Í‰Í‰Ì˜ÍŽÍ•Ì¼Ì£Ì�Í™Ì±ÌŸÌ¹Ì©ÌŸÌ³Ì¦Ì­Í‰Ì®Ì–Ì­Ì£Ì£ÌžÌ™Ì—ÌœÌºÌ­Ì»Ì¥ÍšÍ™Ì�Ì¦Ì²Ì±Í‰Í–Í‰Ì°Ì¦ÍŽÌ«Ì£Ì¼ÍŽÍ�Ì Ì®Í“Ì¹Ì¹Í‰Ì¤Ì°Ì—Ì™Í•Í‡Í”Ì±Í•Ì­ÍˆÌ³Ì—Ì­Í”Ì˜Ì–ÌºÌ®ÌœÌ Í–Ì˜Í“Ì³Í•ÌŸÌ Ì±Ì«Ì¤Í“Í”Ì˜Ì°Ì²Í™Í�Í‡Ì™ÍŽÌ£Ì¼Ì—Ì–Í™Ì¯Í‰Ì ÌŸÍˆÍ�Í•ÌªÍ“Ì�Ì©Ì¦Ì–Ì¹Ì¼Ì Ì˜Ì®ÍšÌŸÍ‰ÌºÌœÍ�Í“Ì¯Ì³Ì±Ì»Í•Ì£Ì³Í‰Ì»Ì­Ì­Ì±Í�ÌªÌ©Ì­ÌºÍ•ÌºÌ¼Ì¥ÌªÍ–Ì¦ÌŸÍŽÌ»Ì°|ÃƒÂ Ã‚ÂºÃ‹â€ |ÃƒÂ¢Ã¢â‚¬â€œÃ¢â‚¬Ëœ|ÃƒÂ¢Ã¢â‚¬â€œÃ¢â€šÂ¬|ÃƒÂ¢Ã¢â‚¬â€œÃ¢â‚¬Å¾|ÃƒÅ’Ã‚Â°ÃƒÅ’Ã‚Â¦ÃƒÅ’Ã‚Â®ÃƒÅ’Ã‚Â eÃƒï¿½Ã‚Â¦ÃƒÅ’Ã…Â¡Ãƒï¿½Ã‚Â¯Ãƒï¿½Ã‚Â¯ÃƒÅ’Ã¯Â¿Â½Ãƒï¿½Ã‚Â®ÃƒÅ’Ã…Â ÃƒÅ’Ã¯Â¿Â½Ãƒï¿½Ã…â€™ÃƒÅ’Ã¢â‚¬Â°Ãƒï¿½Ã¢â‚¬ËœÃƒï¿½Ã‚Â¨ÃƒÅ’Ã…Â Ãƒï¿½Ã¯Â¿Â½Ãƒï¿½Ã¯Â¿Â½ÃƒÅ’Ã‚Â¨ÃƒÅ’Ã…Â¸ÃƒÅ’Ã‚Â¹|UÃƒÅ’Ã‚Â¶ÃƒÅ’Ã‚Â§Ãƒï¿½Ã‚Â©Ãƒï¿½Ã‚Â­Ãƒï¿½Ã‚Â§Ãƒï¿½Ã…Â ÃƒÅ’Ã¢â‚¬Â¦ÃƒÅ’Ã…Â Ãƒï¿½Ã‚Â¥Ãƒï¿½Ã‚Â©ÃƒÅ’Ã‚Â¿ÃƒÅ’Ã¢â‚¬ï¿½ÃƒÅ’Ã¢â‚¬ï¿½Ãƒï¿½Ã‚Â¥Ãƒï¿½Ã…â€™Ãƒï¿½Ã‚Â¬Ãƒï¿½Ã…Â Ãƒï¿½Ã¢â‚¬Â¹Ãƒï¿½Ã‚Â¬Ãƒâ€™Ã¢â‚¬Â°|ÃƒÂ¡Ã‚Â»Ã…â€™ÃƒÅ’Ã‚ÂµÃƒï¿½Ã¢â‚¬Â¡ÃƒÅ’Ã¢â‚¬â€œÃƒÅ’Ã¢â‚¬â€œ|AÃƒÅ’Ã‚Â´Ãƒï¿½Ã¯Â¿Â½ÃƒÅ’Ã‚Â¥ÃƒÅ’Ã‚Â³ÃƒÅ’Ã‚Â ÃƒÅ’Ã…Â¾ÃƒÅ’Ã‚Â¹Ãƒï¿½Ã‚Â©ÃƒÅ’Ã¢â‚¬Â¹ÃƒÅ’Ã¢â‚¬Â Ãƒï¿½Ã‚Â¤Ãƒï¿½Ã¢â‚¬Â¦|EÃƒÅ’Ã‚Â¡ÃƒÅ’Ã¢â‚¬ÂºÃƒï¿½Ã…Â¡ÃƒÅ’Ã‚ÂºÃƒÅ’Ã¢â‚¬â€œÃƒÅ’Ã‚ÂªÃƒï¿½Ã‹â€ ÃƒÅ’Ã‚Â²ÃƒÅ’Ã‚Â»ÃƒÅ’Ã‚Â ÃƒÅ’Ã‚Â°ÃƒÅ’Ã‚Â³ÃƒÅ’Ã¯Â¿Â½ÃƒÅ’Ã‚Â¿)");
	...
	protected void onUserList(String channel, User[] users) {
		if(currentHosts.contains(channel))
		{
			send(targetedChannel, "Thank you " + channel + " for the beautiful " + users.length + " viewer host!  You've earned " + (hostRewardPerViewer*users.length) + " coins for the number of viewers.  They were last playing " + JSONUtil.krakenGame(channel));
			String lChannel = channel.toLowerCase();
			if(currencyMap.get(lChannel) == null)
				currencyMap.put(lChannel,0);
			currencyMap.put(lChannel, currencyMap.get(lChannel) + hostReward + ((hostRewardPerViewer*users.length)));
			partChannel(channel);
		}
	}
	...
	private void updateHosts() {
		for(String host : currentHosts) {
			if(!oldHosts.contains(host) && !hostsThisSession.contains(host)) {
				// This is where we handle a new host.
				// Looks like we need to join their channel and count the chatters that way
				joinChannel(host);
				hostsThisSession.add(host);
			}
		}
		oldHosts = currentHosts;
	}
	...
	private void updateFollows() {
		for(String follower : recentFollows) {
			if(!oldRecentFollows.contains(follower)) {
				// This is where we handle a new follower
				oldRecentFollows.add(follower); // This is gonna grow in size infinitely, but should be able to handle thousands between restarts
				// This should take care of multiple unfollow/follows tho.
				gs.startTimer();
				boolean isLive = JSONUtil.krakenIsLive(targetedChannel.substring(1));
				if(isLive)
					send(targetedChannel, "Thank you so much " + follower + " for the follow!  Sylva's in a glass case of emotion, as you can see.  Here, have " + followCoins + " coins :D");
				int curCoins = 0;
				try{ curCoins = currencyMap.get(follower.toLowerCase()); }
				catch(Exception e) {
					e.printStackTrace();
				}
				currencyMap.put(follower.toLowerCase(), curCoins+followCoins);
			}
		}
		if(oldRecentFollows.size() > 5000) // Arbitrary. 
			oldRecentFollows = recentFollows; // This should help contain it.
	}
	
	private void readRanksFromFile() {
		FileReader fr = null;
		BufferedReader tr = null;
		try {
			rankMap = new HashMap<Integer,String>();
			fr = new FileReader("ranks.txt");
			tr = new BufferedReader(fr);
			String s = "";
			while((s = tr.readLine()) != null) {
				rankMap.put(Integer.parseInt(s), tr.readLine());
			}
			tr.close();
		}
		catch (Exception e){
	...
	private void readCurrencyFromFile() {
		readCurrencyFromFile("currencyInfo.txt", true);
	}
	
	private void consolidateCurrencyInfo(String backupFile) {
		isFinishedReadingCurrency = false;
		HashMap<String,Integer> messedUpCurrencyInfo = currencyMap;
		HashMap<String,Integer> messedUpTimeInfo = timeMap;
		readCurrencyFromFile(backupFile, false); // should put the good old file into currencyMap and timeMap...
		for(String s : messedUpCurrencyInfo.keySet()) {
			if(currencyMap.containsKey(s)) {
				int coins = currencyMap.get(s);
				coins = coins + messedUpCurrencyInfo.get(s);
				currencyMap.put(s, coins);
			}
			else
				currencyMap.put(s, messedUpCurrencyInfo.get(s));
			if(timeMap.containsKey(s)) {
				int mins = timeMap.get(s);
				mins = mins + messedUpTimeInfo.get(s);
				timeMap.put(s, mins);
			}
			else
				timeMap.put(s, messedUpTimeInfo.get(s));
		}
		isFinishedReadingCurrency = true;
		writeCurrencyToFile("currencyInfo.txt");
	}
	
	private void readCurrencyFromFile(String filepath, Boolean startup) {
		...
		try {
			...
			String s = "";
			while((s = tr.readLine()) != null) {
				int coins = Integer.parseInt(tr.readLine());
				currencyMap.put(s.toLowerCase(), coins);
				timeMap.put(s.toLowerCase(), Integer.parseInt(tr.readLine()));
			}
			tr.close();
			isFinishedReadingCurrency = true;
			writeCurrencyToFile("currencyInfoBackup.txt"); // This is kinda pointless but whatever
			if((!timeMap.containsKey("sylvacoin") || timeMap.get("sylvacoin") < 196680) && startup) {
				System.out.println("Something is horribly wrong, sylva doesn't have enough hours - potential wipe detected");
				System.out.println("Attempting to read from currencyInfoOld.txt and ADD it to existing values - rename the backup to this if D didn't provide you this file");
				System.out.println("After this, make sure to change its name so the bot doesn't accidentally add it again next time you start, even though it shouldn't");
				consolidateCurrencyInfo("currencyInfoOld.txt");
			}
			/*fr = new FileReader("ankhbot.csv");
			tr = new BufferedReader(fr);
			s = tr.readLine(); // We want to skip the first line
			while((s = tr.readLine()) != null) {
				String[] results = s.split(",");
				currencyMap.put(results[0].toLowerCase(), Integer.parseInt(results[2]));
				timeMap.put(results[0].toLowerCase(), Integer.parseInt(results[3])*60);
			}*/
		}
		catch (Exception e){
			e.printStackTrace();
	...
	private void writeCurrencyToFile(String filepath) {
		if(!isFinishedReadingCurrency)
			return;
	...
	private void writeCommandCountToFile(String command, Channel channelInfo) {
		PrintWriter writer = null;
		try {
			writer = new PrintWriter(command + "Count.txt","UTF-8");
			int count = channelInfo.getCommandCount(command);
			writer.println(command.toUpperCase() + ": " + count); // I think we want the file to show 'DEATHS 123' like ankhbot
			writer.close();
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	private void readInterestingQuotesFromFile() {
		FileReader fr = null;
		BufferedReader tr = null;
		try {
			fr = new FileReader("interestingQuotes.txt");
			tr = new BufferedReader(fr);
			String s = "";
			while((s = tr.readLine()) != null) {
				interestingQuotes.add(s);
			}
			tr.close();
		}
		catch (Exception e){
		...
		try {
			fr = new FileReader("interestingQuoteLeadIns.txt");
			tr = new BufferedReader(fr);
			String s = "";
			while((s = tr.readLine()) != null) {
				interestingLeadIns.add(s);
			}
			tr.close();
		}
		catch (Exception e){
	...
	private String getUserRank(String s) {
		String curUser = s.toLowerCase();
		int curTime = timeMap.get(curUser);
		String rank = "Coinless";
	...
	protected void onConnect() {
		...
		//Testing for viewing user lists
	...
	protected void onPrivateMessage(String sender, String login,
		...
		Channel channelInfo = getChannelObject(targetedChannel); // Whispers will only apply to the main target channel
		...
		if (BotManager.getInstance().isTagAdmin(sender)
			...
			isAdmin = true;
		if (targetedChannel.equalsIgnoreCase("#" + sender))
			isOwner = true;
		...
		// !command - Ops
				if ((msg[0].equalsIgnoreCase(prefix + "command") || (msg[0]
					...
					if (msg.length < 3) {
						...
						if (msg[1].equalsIgnoreCase("add") && msg.length > 3) {
							...
							sendPM(sender, "Command added/updated.");
							...
							channelInfo.removeScheduledCommand(key);
							if (removed) {
								sendPM(sender, "Command " + key + " removed.");
							} else
								sendPM(sender, "Command " + key + " doesn't exist.");
							...
							if (channelInfo.getCommand(command) != null) {
								...
								if (channelInfo.setCommandsRestriction(command, level))
									sendPM(sender, prefix + command + " restricted to "
											+ levelStr + " only.");
								else
									sendPM(sender, "Error setting restriction.");
							} else {
								sendPM(sender, "Command does not exist.");
							}
							...
							boolean result = channelInfo.renameCommand(key, newKey,
									sender);
							if (result) {
								sendPM(sender, "Command " + key + " renamed to "
										+ newKey + " successfully.");
							} else {
								sendPM(sender,
							...
							if (editor != null && !editor.equals("-1")) {
								...
								sendPM(sender, "That command does not exist.");
							} else {
								sendPM(sender,
				...
				if (msg[0].equalsIgnoreCase(prefix + "schedulelist") && isOp) {
					if(msg[1].equalsIgnoreCase("add")) {
						channelInfo.scheduleList.add(msg[2]);
						sendPM(sender, "Added command " + msg[2] + " to the schedule list");
						channelInfo.saveScheduledCommands();
					}
					
					if(msg[1].equalsIgnoreCase("list")) {
						String outputString = "Scheduled commands: ";
						for(String s : channelInfo.scheduleList) {
							outputString+=(s + ", "); // leaves a comma on the last one but idgaf
						}
						sendPM(sender, outputString);
					}
					
					if(msg[1].equalsIgnoreCase("settimer")) {
						try {
							channelInfo.scheduleListTimer = Integer.parseInt(msg[2]);
							channelInfo.saveScheduledCommands();
							sendPM(sender, "Schedule list timer set to occur every " + msg[2] + " minutes");
						}
						catch (Exception e) {
							sendPM(sender, "Invalid number: " + msg[2]);
						}
					}
					
					if(msg[1].equalsIgnoreCase("setmessagedelay")) {
						try {
							channelInfo.scheduleListMessageDiff = Integer.parseInt(msg[2]);
							channelInfo.saveScheduledCommands();
							sendPM(sender, "Schedule list message delay set to " + msg[2] + " messages");
						}
						catch(Exception e) {
							sendPM(sender, "Invalid number: " + msg[2]);
						}
					...
					if(msg[1].equalsIgnoreCase("remove")) {
						if(channelInfo.scheduleList.contains(msg[2]))
						{
							channelInfo.scheduleList.remove(msg[2]);
							channelInfo.saveScheduledCommands();
							sendPM(sender, "Successfully removed " + msg[2] + " from schedule list");
						}
						else
						{
							sendPM(sender, msg[2] + " is not in the schedule list");
						}
					}
				}
	}
	...
	protected void onMessage(String channel, String sender, String login,
		...
		if (tags != null) {
			// Wow, tags got all fucked up somewhere
			// color is 1, displayname 2, emotes 3, subscriber 6 (or 7 judging by ends), turbo 7, usertype 9
			String color = tags[1].substring(tags[1].indexOf("=") + 1);
			String displayName = tags[2].substring(tags[2].indexOf("=") + 1);
			String emotes = tags[3].substring(tags[3].indexOf("=") + 1);
			String subscriber = tags[6].substring(tags[6].indexOf("=") + 1);
			...
			String turbo = tags[7].substring(tags[7].indexOf("=") + 1);
			...
			String userType;
			if (!tags[9].endsWith("=")) {
				userType = tags[9].substring(tags[9].indexOf("=") + 1);
	...
	protected void onChannelMessage(String channel, String targetChannel,
		...
		// Chatterbot, puns, Cmenbot etc.
		if(msg[0].equalsIgnoreCase(prefix + "ctof")) {
			double celsius = Double.valueOf(msg[1]);
			try {
				send(channel, celsius + "°C = " + String.format("%.1f",(celsius*1.8+32)) + "°F");
			}
			catch (Exception e) {
				send(channel, "No.  You did it wrong.");
			}
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "ftoc")) {
			double fahrenheit = Double.valueOf(msg[1]);
			try {
				send(channel, fahrenheit + "°F = " + String.format("%.1f",(fahrenheit-32)/1.8) + "°C");
			}
			catch (Exception e) {
				send(channel, "No.  You did it wrong.");
			}
		...
		if(msg[0].equalsIgnoreCase(prefix + "trashtalk") && isOp){
			String fileName = "shittalk.txt";
			String line = null;
			int numLines = 481;
			int targetLine = (int) (Math.random()*numLines);
			try{
				...
				if(targetLine == 0)
					line = bReader.readLine(); // I'm lazy
				bReader.close();
				send(channel, line);
			}
			catch(Exception e) {
		...
		if(msg[0].equalsIgnoreCase("!pun")) {
			// http://www.punoftheday.com/cgi-bin/randompun.pl
			URL url;
			String readStream = "";
			try {
				url = new URL("http://www.punoftheday.com/cgi-bin/randompun.pl");
			...
			int position = readStream.indexOf("class=\"dropshadow1\">") + "class=\"dropshadow1\">".length();
			readStream = readStream.substring(position+3);
			position = readStream.indexOf("</p>");
			readStream = readStream.substring(0, position);
			send(channel,readStream);
		}
		
		
		if (msg[0].equalsIgnoreCase(this.getName() + ",") && bot1session != null) {
			try {
				send(channel, bot1session.think(fuseArray(msg, 1)));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		if (msg[0].equalsIgnoreCase(prefix + "give")) {
			if(msg.length < 3) {
				send(channel, "Incorrect syntax; " + prefix + "give [username] [amount]");
				return;
			}
			int amount = 0;
			try {
				amount = Integer.parseInt(msg[2]);
			} catch (Exception e)
			{
				send(channel, "Error processing amount: " + msg[2]);
				return;
			}
			System.out.println("Got here...");
			if(!currencyMap.keySet().contains(msg[1].toLowerCase()))
			{
				send(channel, "You can't give coins to someone I've never seen before! (You're welcome)");
				return;
			}
			System.out.println("And here...");
			int giverCurrencyTotal = currencyMap.get(sender.toLowerCase());
			if(giverCurrencyTotal >= amount) {
				int receiverCurrencyTotal = currencyMap.get(msg[1].toLowerCase());
				currencyMap.put(sender.toLowerCase(), giverCurrencyTotal - amount);
				currencyMap.put(msg[1].toLowerCase(), receiverCurrencyTotal + amount);
				send(channel, "Success! " + sender + ": " + giverCurrencyTotal + "-" + amount + "=" + (giverCurrencyTotal-amount) + " coins ; "
						+ msg[1] + ": " + receiverCurrencyTotal + "+" + amount + "=" + (receiverCurrencyTotal+amount) + " coins");
			}
			else
				send(channel, "You do not have enough coins for that!");
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "coins")) {
			int coins = currencyMap.get(sender.toLowerCase());
			send(channel, sender + ", you have " + coins + " coins!");
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "modgive") && isOp) {
			if(msg.length < 3) {
				send(channel, "Incorrect syntax; " + prefix + "modgive [username] [amount]");
				return;
			}
			int amount = 0;
			try {
			...
			int curCoins = currencyMap.get(msg[1].toLowerCase());
			currencyMap.put(msg[1].toLowerCase(), curCoins + amount);
			send(channel, "Success! Generated " + amount + " coins out of thin air; " + msg[1] + " now has " + (curCoins+amount) + " coins!");
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "modgiveall") & isOp) {
			try {
				int numToAdd = Integer.parseInt(msg[1]);
				for(User u : instance.getUsers(targetedChannel)) {
					int curCoins = currencyMap.get(u.getNick());
					currencyMap.put(u.getNick(),curCoins + numToAdd);
				}
				send(channel, "Successfully added " + msg[1] + " coins to all " + instance.getUsers(targetedChannel).length + " users in chat!");
			}
			catch(Exception e) {
				send(channel, "Invalid number: " + msg[1]);
			}
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "raiders") && isOwner) {
			String returnString = "Awarded " + raidReward + " coins to each of the following: ";
			for(int i = 1; i < msg.length; i++) {
				String s = msg[i].toLowerCase();
				currencyMap.put(s, currencyMap.get(s)+raidReward);
				returnString = returnString + msg[i] + ", ";
			}
			send(channel, returnString);
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "shutdown") && isOwner) {
			send(channel, "What... really?  Okay... I guess... Goodbye, cruel world.");
			writeCurrencyToFile("currencyInfo.txt");
			System.exit(0);
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "rank")) {
			// Get text rank...
			String rank = getUserRank(sender);
			send(channel, sender + ", you are rank " + rank + " with " + timeMap.get(sender.toLowerCase()) + " minutes!  You don't want me to convert that to hours, trust me.");
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "say") && isOwner) {
			send(channel, fuseArray(msg,1));
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "interestingquote")) {
			send(channel,interestingLeadIns.get((int)Math.floor(Math.random()*(interestingLeadIns.size()+1))) + interestingQuotes.get((int)Math.floor(Math.random()*(interestingQuotes.size()+1))));
		}
		
		if(msg[0].equalsIgnoreCase(prefix + "greenscreen") && isOwner) {
			gs.setVisible(true);
			gs.setState(Frame.NORMAL);
		}
		...
		// *************************************
		// ********************************************************************************
		if ((msg[0].equalsIgnoreCase(prefix + "raffle") || msg[0].equalsIgnoreCase(prefix + "giveaway")) && msg.length == 1) {
			log("RB: Matched command !raffle (user entry)");
			if (channelInfo.raffle != null) {
				String curUser = sender.toLowerCase();
				int curCoins = currencyMap.get(curUser);
				if(curCoins >= raffleCost) {
					currencyMap.put(curUser, curCoins - raffleCost);
					channelInfo.raffle.enter(sender); // Ask sylva if he wants to have a response on this.  I'm putting it in whisper.
					send(channel, "/w " + sender + " You have successfully entered!");
					return;
				}
				else
					send(channel, "You don't have enough coins to enter! It costs " + raffleCost + " and you have " + curCoins);
				return;
			}
		}
		// ********************************************************************************
		...
		// if (msg.length > 1) {
		// send(channel, "(>ÃƒÂ Ã‚Â²Ã‚Â _ÃƒÂ Ã‚Â²Ã‚Â )> " + fuseArray(msg, 1));
		// } else
		...
		if (msg[0].equalsIgnoreCase(prefix + "uptime") && channelInfo.getShouldModerate()) {
		...
		// Removed for conflict with a custom command
		// !raid commands
		...
		// if (msg.length > 1) {
		//
		// send(channel, " (ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â¯Ãƒâ€šÃ‚Â°ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¡Ãƒâ€šÃ‚Â°ÃƒÂ¯Ã‚Â¼Ã¢â‚¬Â°ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â¯ÃƒÂ¥Ã‚Â½Ã‚Â¡ " +
		// fuseArray(msg, 1));
		// }
		//
		// }
		...
		// !scheduleList - Ops
		// This should be able to add commands to a list, and the bot will read from that list in order every X minutes, as established by the command
		/*
		 * Channel channelInfo = BotManager.getInstance().getChannel(channel);
            System.out.println("Message count is " + channelInfo.messageCount + " and last count is " + ScheduledCommand.this.lastMessageCount);
            if (channelInfo.messageCount - ScheduledCommand.this.lastMessageCount >= messageDifference) {
		...
		if (msg[0].equalsIgnoreCase(prefix + "schedulelist") && isOp) {
			if(msg[1].equalsIgnoreCase("add")) {
				channelInfo.scheduleList.add(msg[2]);
				send(channel, "Added command " + msg[2] + " to the schedule list");
				channelInfo.saveScheduledCommands();
			}
			...
			if(msg[1].equalsIgnoreCase("settimer")) {
				try {
					channelInfo.scheduleListTimer = Integer.parseInt(msg[2]);
					channelInfo.saveScheduledCommands();
					send(channel, "Schedule list timer set to occur every " + msg[2] + " minutes");
				}
				catch (Exception e) {
					send(channel, "Invalid number: " + msg[2]);
				}
			}
			...
			if(msg[1].equalsIgnoreCase("setmessagedelay")) {
				try {
					channelInfo.scheduleListMessageDiff = Integer.parseInt(msg[2]);
					channelInfo.saveScheduledCommands();
					send(channel, "Schedule list message delay set to " + msg[2] + " messages");
				}
				catch(Exception e) {
					send(channel, "Invalid number: " + msg[2]);
				}
			...
			if(msg[1].equalsIgnoreCase("remove")) {
				if(channelInfo.scheduleList.contains(msg[2]))
				{
					channelInfo.scheduleList.remove(msg[2]);
					channelInfo.saveScheduledCommands();
					send(channel, "Successfully removed " + msg[2] + " from schedule list");
				}
				else
				{
					send(channel, msg[2] + " is not in the schedule list");
				}
			}
		}
		...
		// !schedule - Ops
		// removed in favor of !schedulelist
		...
		if (msg[0].equalsIgnoreCase(prefix + "autoreply") && isOp) {
			...
			if (msg.length < 3) {
				if (msg.length > 1 && msg[1].equalsIgnoreCase("list")) {
					 ...
					 .replaceAll("\\\\Q", "")
					 .replaceAll("\\\\E", "");
					 resultString = resultString + cleanedTrigger + " (" + i + ")";
					 if(i != channelInfo.autoReplyTrigger.size()-1)
						 resultString+=",";
					 }
					send(channel,
							resultString);
		...
		// !raffle - Ops
		if ((msg[0].equalsIgnoreCase(prefix + "raffle") || msg[0].equalsIgnoreCase(prefix + "giveaway")) && isOp) {
			log("RB: Matched command !raffle");
			if (msg.length >= 2) {
				if (msg[1].equalsIgnoreCase("start")) {
					if(msg.length < 5) {
						send(channel, "Invalid syntax.  " + prefix + "giveaway start [Cost] [Sponsor] [Description]");
						return;
					}
					try {
						raffleCost = Integer.parseInt(msg[2]);
					}
					catch (Exception e) {
						send(channel, "Unable to use a cost of " + msg[2]);
						return;
					}
					raffleSponsor = msg[3];
					raffleReward = fuseArray(msg,4);
					send(channel, "A giveaway has started! Enter to win " + raffleReward + ", sponsored by " + raffleSponsor + "!  "
							+ "Type !giveaway to enter - entry fee is " + raffleCost + " coins");
					if (channelInfo.raffle == null) {
					...
					Runnable raffleSpammer = new Runnable() {
						...
						public void run() {
							...
							if(raffleRunning) {
								send(targetedChannel, "A giveaway is in progress! Enter to win " + raffleReward + ", sponsored by " + raffleSponsor + "!  "
										+ "Type !giveaway to enter - entry fee is " + raffleCost + " coins"); // This is gross but whatever (targetedchannel)
								Thread t = new Thread(this);
								t.start();
							}
						}
					...
					Thread t = new Thread(raffleSpammer);
					t.start();
					
				} else if (msg[1].equalsIgnoreCase("cancel")) {
					if (channelInfo.raffle != null) {
						channelInfo.raffle.setEnabled(false);
						raffleRunning = false;
					}

					send(channel, "Giveaway cancelled.");
					...
					send(channel, "Giveaway entries cleared.");
				} else if (msg[1].equalsIgnoreCase("count")) {
					if (channelInfo.raffle != null) {
						send(channel,
								"Giveaway has " + channelInfo.raffle.count()
										+ " entries.");
					} else {
						send(channel, "Giveaway has 0 entries.");
					}
				} else if (msg[1].equalsIgnoreCase("winner") || msg[1].equalsIgnoreCase("stop") || msg[1].equalsIgnoreCase("end") || msg[1].equalsIgnoreCase("reroll")) {
					if (channelInfo.raffle != null) {
						send(channel,
								"The winner is " + channelInfo.raffle.pickWinner()
										+ "!  You have won " + raffleReward + " thanks to " + raffleSponsor + "! "
												+ "Get in touch with " + raffleSponsor + " via PM or whisper for your prize");
						channelInfo.raffle.setEnabled(false);
						raffleRunning = false;
					} else {
				...
				System.out.println(" Got here with user " + sender);
				if (channelInfo.raffle != null) { // Make sure this is running...
					System.out.println("And here");
		...
		if (msg[0].substring(0, 1).equalsIgnoreCase(prefix)) {
			...
			if (value != null) {
				...
				if(msg.length > 1) {
					if(msg[1].equalsIgnoreCase("+") && isOp) {
						channelInfo.increaseCommandCount(command);
						changedCount = true;
					}
					if(msg[1].equalsIgnoreCase("-") && isOp) {
						channelInfo.decreaseCommandCount(command);
						changedCount = true;
					}
					
					if(msg[1].equalsIgnoreCase("set") && isOp) {
						try {
							int newCount = Integer.parseInt(msg[2]);
							channelInfo.setCommandCount(command,  newCount);
							changedCount = true;
						} catch (Exception e) { send(channel, "Invalid number: " + msg[2]); }
					}
				}
				...
				if (channelInfo.checkCommandRestriction(command, accessLevel)) {
					...
					if (currentTime > (lastCommand + cooldown * 1000L) || isOp) {
						...
						//if(!changedCount)
						//	channelInfo.increaseCommandCount(command);
						// Removed since we now manually increase/decrease this count
						if(changedCount)
							writeCommandCountToFile(command, channelInfo);
						send(channel, sender, value);
	...
	public void onPart(String channel, String sender, String login,
		...
		if (channelInfo == null)
			return;
		System.out.println("Someone just left, " + sender);
	}
	...
	public void sendPM(String target, String message) {
		send(targetedChannel, null, "/w " + target + " " + message, null);
	}
	...
	public void send(String target, String sender, String message, String[] args) {
		if (msgTimer.size() > 19) {
			...
			if (diff > 30 * 1000L) {
				...
				if (message.startsWith("/me ") || message.startsWith("/w "))
					useBullet = false;
