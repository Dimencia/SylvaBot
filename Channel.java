...
public class Channel {
	...
	public int scheduleListTimer = 1; // Default 5 minutes, set to 1 for testing
	public int scheduleListMessageDiff = 10; // Default 10 messages
	public ArrayList<String> scheduleList = new ArrayList<String>();
	private int scheduleListCounter = 0;
	...
	public Channel(String name) {
		...
		try {
			Object obj = parser.parse(new BufferedReader(new InputStreamReader(new FileInputStream(channel + ".json"), "UTF-8")));
		...
		Runnable scheduleListRunner = new Runnable() {

			@Override
			public void run() {
				try {
					System.out.println("Hitting schedule timer, will delay " + (scheduleListTimer*60000) + " miliseconds before next one");
					if(ReceiverBot.getInstance() == null) {
						System.out.println("Skipping because bot is null");
					}
					else {
						Channel channelInfo = BotManager.getInstance().getChannel(channel);
						System.out.println(channelInfo.messageCount + " is the current count, old count is " + lastMessageCount);
						if (channelInfo.messageCount - lastMessageCount >= scheduleListMessageDiff) {
							ReceiverBot.getInstance().send(channel, channelInfo.getCommand(scheduleList.get(scheduleListCounter)));
							lastMessageCount = (int) channelInfo.messageCount;
							scheduleListCounter++;
						} // Move this last line down below the if you want it X messages per Y minutes, but I'd rather X messages total since last sent schedule
	
						if(scheduleListCounter >= scheduleList.size()) 
							scheduleListCounter = 0;
					}
					Thread.sleep(scheduleListTimer*60000);
				} catch (Exception e) {
					e.printStackTrace();
				}
				Runnable r = this;
		...
		Thread scheduleListThread = new Thread(scheduleListRunner);
	...
	public void decreaseCommandCount(String commandName) {
		commandName = commandName.toLowerCase();
		if (commandCounts.containsKey(commandName)) {
			int currentCount = commandCounts.get(commandName);
			currentCount--;
	...
	public void setCommandCount(String commandName, int count) {
		commandName = commandName.toLowerCase();
		if (commandCounts.containsKey(commandName)) {
			int currentCount = commandCounts.get(commandName);
			currentCount = count;
	...
	public int getCommandCount(String commandName) {
		commandName = commandName.toLowerCase();
		if (commandCounts.containsKey(commandName)) {
			return commandCounts.get(commandName);
		}
		return 0;
	}
	
	public void resetCommandCount(String commandName) {
		commandName = commandName.toLowerCase();
		if(commandCounts.containsKey(commandName)) {
			commandCounts.put(commandName, 0);
		}
		saveCommands(false);
	}
	...
	public void setScheduledCommand(String key, String delay, int diff) {
		if (commandsSchedule.containsKey(key)) {
			commandsSchedule.get(key).s.stop();
			commandsSchedule.remove(key);
			ScheduledCommand rc = new ScheduledCommand(channel, key, delay,
					diff);
	...
	public void saveScheduledCommands() {

		JSONArray scheduledCommands = new JSONArray();
		ReceiverBot bot = ReceiverBot.getInstance();
		
		Iterator itr = scheduleList.iterator();
		
		while (itr.hasNext()) {
			String commName = (String) itr.next();
			JSONObject scheduleObj = new JSONObject();
			scheduleObj.put("name", commName);
			scheduleObj.put("delay",
					scheduleListTimer);
			scheduleObj.put("messageDifference",
					scheduleListMessageDiff);
	...
	/*
	 * https://api.twitch.tv/kraken/oauth2/authorize?response_type=token&client_id=77pqtc6kd4m0qy34np2ysw33na04ium&redirect_uri=http://localhost&scope=user_read user_blocks_edit user_blocks_read user_follows_edit channel_read channel_editor channel_stream channel_subscriptions user_subscriptions channel_check_subscription chat_login channel_feed_read channel_feed_edit
	 * 
	 */
	...
	public boolean updateGame(String game) throws IOException {
		...
		System.out.println(BotManager.getInstance().krakenOAuthToken);
		String request = "{"
				+ "\"channel\": {"
				+ "\"game\": \"" + JSONObject.escape(game) + "\""
						+ "}"
						+ "}"; // I edited some of this when I didn't realize I didn't have my kraken oauth and client ID... hope it didn't break.
		String resp = BotManager.putRemoteData(
				"https://api.twitch.tv/kraken/channels/"
						+ this.channel.substring(1),request);
		...
		try {
			...
			try {
				...
				System.out.println("Current game after changing is " + newStatus);
	...
	private void loadProperties(String name) {
		 ...
		 * scheduleObj.put("name", commName);
			scheduleObj.put("delay",
					bot.scheduleListTimer);
			scheduleObj.put("messageDifference",
					bot.scheduleListMessageDiff);
		 */
		for (int i = 0; i < scheduledCommandsArray.size(); i++) {
			JSONObject scheduledCommandsObj = (JSONObject) scheduledCommandsArray
					.get(i);
			ReceiverBot bot = ReceiverBot.getInstance();
			long goddam = (long) scheduledCommandsObj.get("messageDifference");
			scheduleListMessageDiff = (int) goddam;
			if(scheduledCommandsObj.get("delay") == null)
				goddam = 5; // default 5 min... we convert to miliseconds in the timer
			else
				goddam = (long) scheduledCommandsObj.get("delay");
			scheduleListTimer = (int) goddam;
			scheduleList.add(((String) scheduledCommandsObj.get("name")).replaceAll(
							"[^a-zA-Z0-9]", ""));
		}
	...
	public void saveConfig(Boolean shouldSendUpdate) {
		try {
			File filepath = new File(channel + ".json");
			OutputStreamWriter file = new OutputStreamWriter(new FileOutputStream(filepath),"UTF-8");
